...
like ICON store their fields on an unstructured mesh: a variable has a single
cell dimension (e.g. `ncells`), and the position of each cell is given by a
pair of coordinate arrays (`clon`/`clat`) of the same length. CDFViewer
handles such data transparently — you select `clon` and `clat` as plot axes
like any other coordinate, and the cells are resampled onto a regular grid
behind the scenes.
...
Unstructured cells cannot be drawn as a regular image directly, so CDFViewer
resamples them with nearest-neighbor interpolation onto a regular grid. By
default, each axis gets **500 sample points** spanning the full range of the
coordinate.
...
in a separate grid file. When CDFViewer detects such missing coordinates, it
attaches them from a grid file.
...
Every match is verified against the data file — the grid uuid and the
dimension sizes must fit — before it is used. Automatic search can be
disabled with `--no-grid-search`.
