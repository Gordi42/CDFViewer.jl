...
CDFViewer writes three kinds of output: still images of the current figure,
video recordings of an animation, and a command-line string that reproduces
the whole session. All three are available as REPL commands and as the
...
- `filename` — output path; the `.png` extension is added if missing. If
  the file already exists, a number is appended (`temperature(1).png`) so
  nothing is overwritten.
- `px_per_unit` — resolution multiplier: `2` doubles the pixel resolution
  at the same figure layout, for print-quality output.
...
- `filename` — output path, extension selects the codec,
- `framerate` — frames per second (default 30),
- `range` — restrict the recorded frames, e.g. `range=1:12`.

The animated dimension is set with `pdim` (or the *Play* dropdown); see
[Animation and Playback](animation.md).

## Batch mode: `--savefig` and `--record`
...
## Reproducing a session: `export`

After interactively tuning a plot, `export` prints the command line that
recreates the current state — variable, axes, plot type, fixed indices,
keyword arguments, and even the current axis limits:
