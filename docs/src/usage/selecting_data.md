...
Every plot in CDFViewer is defined by three choices. You pick which variable
to show, which dimensions go on the plot axes, and at which index the
...
variable together with their currently selected values, and `varinfo`
prints the full metadata of a variable.
...
assign determines which plot types are available. One axis allows line and
scatter plots, two allow heatmaps and contours, and three allow volume
rendering (see [Plot Types](plot_types.md)).
...
- `sel <dim> <value>` selects by coordinate **value** and snaps to the
  nearest grid point.
